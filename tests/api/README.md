...
`npm run test:schemathesis` runs a deterministic bounded coverage profile against `SCHEMATHESIS_SPEC` and `SCHEMATHESIS_BASE_URL` (defaulting to `http://127.0.0.1:3412/openapi.json` and `http://127.0.0.1:3412`). CI supplies a temporary Python environment and keeps the JUnit report under `artifacts/`, including when checks fail. The profile uses seed `20260912`, one worker, at most 20 examples per operation, a 10-second total budget, and a 1-second request deadline. It checks server errors, status codes, content types, and response schemas; broader fuzzing/stateful runs are manual or scheduled, while state-sensitive positive/negative acceptance stays in the exact HTTP suite. Set `SCHEMATHESIS_AUTH_TOKEN` to exercise protected operations with a sanitized bearer header; exact authenticated workflows remain in `test:api` because login tokens and order state are business fixtures.
